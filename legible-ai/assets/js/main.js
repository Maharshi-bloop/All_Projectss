...
$(document).ready(function () {
    ...
    $("nav > ul > li").on("click", function () {
     ...
     $("nav > ul > li").each(function () {
        if ($(this).children(".bigMenu").length > 0) {
            $(this).addClass("hasUl");
            $(this).children(".bigMenu").addClass("subMenu");
        }
    });
     $("nav > ul > li").on("click", function (e) {
        e.stopPropagation();
        $(this).toggleClass("activeLi").siblings().removeClass("activeLi");
    });
    ...
    if ($(window).width() <= 1024) {
         $("nav > ul > li > .bigMenu").slideUp();
        $("nav > ul > li")
            .off("click")
            .on("click", function (e) {
                e.stopPropagation();
                $(this).siblings().find(".bigMenu").slideUp();
                if ($(this).children(".bigMenu").length > 0) {
                    $(this).children(".bigMenu").stop().slideToggle();
                }
            });
    }
    ...
    $(".workflowsFilterTop li").on("click", function (e) {
        e.preventDefault();
        $(".workflowsFilterTop li").removeClass("active");
        $(this).addClass("active");
    });
