...
jQuery(document).on('click', '.productList', function (e) {

    e.preventDefault();
    e.stopPropagation();

    let target = jQuery(this).data('product'); // ✅ FIX

    if (!target) return;

    jQuery('.productList').removeClass('activeProduct');
    jQuery('.productDetailOuter').removeClass('activepopUp');

    jQuery(this).addClass('activeProduct');

    jQuery('.productDetailOuter[data-target="' + target + '"]').addClass('activepopUp');
