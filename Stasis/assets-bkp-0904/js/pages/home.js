...
if ($(window).width() >= 480) {
    ...
    function setWidths($active) {
        ...
        const openWidth = wrapperWidth * 0.35; 
        const otherWidth = (wrapperWidth * (1 - 0.35)) / (totalCount - 1);
    ...
    // 👉 HOVER HANDLER
    $accordions.on("mouseenter", function () {
        setWidths($(this));
    });

    // RESPONSIVE FIX
    $(window).on("resize", function () {
        const $active = $(".industriesServedList.open");
        setWidths($active.length ? $active : $accordions.first());
    });
}
